...
Click the "Working Memory" tab in the debugger UI to see a list containing Samantha's working memory:

![](/images/guide/run-example-soul/05-working-memory.png)

A working memory is a list of memories that are created as cognitive steps are executed. So when Samantha thinks, speaks, or makes a decision, a new corresponding memory is created and added to her working memory. These memories are stored in a special `WorkingMemory` object.
