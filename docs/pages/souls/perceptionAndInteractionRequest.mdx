...
Your `MentalProcess` can dispatch _metadata as well. By default, every step within `WorkingMemory` will dispatch `action:"says"` until otherwise specified by you. When adding custom actions, remember to extract `dispatch` from `useActions()`.
