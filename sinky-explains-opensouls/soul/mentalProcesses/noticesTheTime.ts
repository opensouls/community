...
const noticesTheTime: MentalProcess = async ({ step }) => {
  ...
  if (time.getTime() - timeOfLastUserMessage.getTime() > 30 * 1000) {
    return step.next(externalDialog("Ask the user if you're boring them? Slinky says something interesting about themself."))
  }
